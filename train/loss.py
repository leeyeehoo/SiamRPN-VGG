...
def rpn_cross_entropy_balance(input, target, num_pos, num_neg):
    ...
    pos_loss = F.cross_entropy(input=input.reshape(-1, 2)[cal_index_pos], target=target.view(-1)[cal_index_pos]) / cal_index_pos.shape[0]
    neg_loss = F.cross_entropy(input=input.reshape(-1, 2)[cal_index_neg], target=target.view(-1)[cal_index_neg]) / cal_index_neg.shape[0]
